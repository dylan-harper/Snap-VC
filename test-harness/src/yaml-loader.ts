...
function parseStep(value: unknown, location: string): Step {
  ...
  switch (tag) {
    ...
    case "assert": {
      ...
      return {
        type: "assert",
        assertions: values.map((item, i) => stateAssertion(item, `${location}.assert[${i}]`)),
      };
    }
    default:
...
function startStep(value: unknown, location: string): StartStep {
  ...
  if (ready.capture !== undefined) {
    ...
    capture = {
      as,
      ...(captureMap.group === undefined
        ? {}
        : { group: nonnegative(captureMap.group, `${location}.ready.capture.group`) }),
    };
  }
  return {
...
function startHttpStep(value: unknown, location: string): StartHttpStep {
  ...
  return {
    ...
    routes: array(map.routes, `${location}.routes`).map((route, i) =>
      httpRoute(route, `${location}.routes[${i}]`),
    ),
  };
}
  ...
  switch (type) {
    case "tree_equals": {
      ...
      const entries = array(map.entries, `${location}.entries`).map((entry, i) =>
        treeEntry(entry, `${location}.entries[${i}]`),
      );
    ...
    case "trees_equal":
      ...
      return {
        ...
        ...(map.ignore === undefined ? {} : { ignore: strings(map.ignore, `${location}.ignore`) }),
      };
    case "http_requests_equal": {
      ...
      const requests = array(map.value, `${location}.value`).map((item, i) => {
        ...
        return {
          method: method(request.method, `${location}.value[${i}].method`),
          target: string(request.target, `${location}.value[${i}].target`),
        };
      });
